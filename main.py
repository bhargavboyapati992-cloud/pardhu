...
async def startup_event():
    ...
    try:
        ...
        for u in users:
            send_sms(u.phone_number, "≡ƒî▒ Smart Plant Watering System Simulator has started!")
...
def receive_sensor_data(data: crud.SensorDataCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if MOTOR_STATE["mode"] == "Auto":
        ...
        if decision == "TURN_ON" and not MOTOR_STATE["is_on"]:
            ...
            users = crud.get_users(db)
            for u in users:
                background_tasks.add_task(send_sms, u.phone_number, "ΓÜá Soil is dry (below 600). Motor turned ON automatically.")
            ...
            users = crud.get_users(db)
            for u in users:
                background_tasks.add_task(send_sms, u.phone_number, "Γ£à Soil fully saturated (1023). Motor turned OFF automatically.")
